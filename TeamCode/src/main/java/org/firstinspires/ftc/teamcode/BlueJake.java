...
public class BlueJake extends LinearOpMode {
    ...
    int LIFT_LOW  = 500;   // adjust to your robot
    int LIFT_HIGH = 2000;  // adjust to your robot
    long lastClick = 0;
...
// --- inside loop ---
            ...
            if (Math.abs(liftPower) > 0.05) {
                ...
                int target = (now - lastClick < 400) ? LIFT_HIGH : LIFT_LOW;
                // if time since last click < 400 ms → treat as double-click → go HIGH
                // else (first click or slow click) → go LOW
                lastClick = now;
